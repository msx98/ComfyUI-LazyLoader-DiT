...
def _load_sibling(module, reader, checkpoint_prefix, key_transform, device):
    ...
    for name, tensor in list(parameters.items()) + list(buffers.items()):
        ...
        _dtype_copy_(destination, source)
        del source
        _set_qualified(
            module, name,
...
def load_lazy_qwen_text_encoder(clip_path, memory_limit_mib, debug=False, profile_name="auto", prefetch=True):
    ...
    for path, prefix in profile.resident_siblings:
        ...
        loaded, skipped = _load_sibling(module, reader, prefix, profile.key_transform, device)
